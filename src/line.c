...
int	ind_dollar(int i, char **mat, char *line)
{
	...
	prep_split = prepare_split(line);
	j = -1;
	if (ft_strchr(mat[i], '$') && ft_strlen(mat[i]) > 1 && !ft_strchr(prep_split[i], 's'))
	{
		while(mat[i][++j])
			if (mat[i][j] == '$')
				break ;
		if (mat[i][j] == '$')
		return (ft_free(prep_split), 1);
	}
	ft_free(prep_split);
	return (0);
}

char	**mini_change_mat(char **mat, int i, char **src, char *line)
{
	char	*tmp;
	int		j;

	j = ft_strlen(line); ////////////////////////////////////BORRAR
	j = -1;
	while (mat[++i])
	{
